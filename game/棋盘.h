...
2023.12.21可以自定义搜索分支数和深度
2025.05.06增加了棋盘初始化功能，方便后续扩展
*/
#include<bits/stdc++.h>
#ifndef FiveChessSize
#define FiveChessSize 15
#endif
//FiveChessSize为棋盘的边长，可以修改为5~181之间的任意正整数值，推荐值为15
#ifndef SearchNum
#define SearchNum 9
#endif
...
namespace FiveChess{
	class chessboard{//棋盘类
		...
		public:
			...
			chessboard(short**B){//棋盘初始化为目标数组
				for(int i=0;i<FiveChessSize;i++)for(int j=0;j<FiveChessSize;j++)b[i][j]=B[i][j];
			}
			void restart(){//棋盘重置(全空)
				memset(b,0,sizeof(b));
			}
			void set(short**B){//棋盘重置为目标数组
				for(int i=0;i<FiveChessSize;i++)for(int j=0;j<FiveChessSize;j++)b[i][j]=B[i][j];
			}
			...
			friend bool operator==(const chessboard &x,const chessboard &y);
			friend bool operator!=(const chessboard &x,const chessboard &y);
	...
	bool operator==(const chessboard &x,const chessboard &y){//判断两个棋盘是否相同
		for(short i=0;i<FiveChessSize;i++)for(short j=0;j<FiveChessSize;j++)if(x.b[i][j]!=y.b[i][j])return 0;
		return 1;
	}
	bool operator!=(const chessboard &x,const chessboard &y){//判断两个棋盘是否不同
		for(short i=0;i<FiveChessSize;i++)for(short j=0;j<FiveChessSize;j++)if(x.b[i][j]!=y.b[i][j])return 1;
		return 0;
	}
	class chessAI_static{//静态评估落子算法
		...
		protected:
			...
			inline long long value(chessboard &cb,const short &x,const short &y){//落子(x,y)估值函数
				int p=0,n=0;
	...
	class chessAI_dfs_static:public chessAI_static{//基于静态评估函数剪枝的深度优先搜索
		...
		private:
			...
			inline long long value_enemy(chessboard &cb,const unsigned short &x,const unsigned short &y){//落子(x,y)对方估值函数
				int p=0,n=0;
				cb.find(x>4?x-4:0,x+5<FiveChessSize?x+5:FiveChessSize,y>4?y-4:0,y+5<FiveChessSize?y+5:FiveChessSize,121,5,p,n);
				long long ret=r==-1?(p*success5[0]-n*success5[1]):(n*success5[0]-p*success5[1]);
				p=0;n=0;
		...
		public:
			std::pair<unsigned short,unsigned short> put(chessboard cb){//基于深度优先搜索和静态估值函数进行落子
				unsigned short ep=cb.empty_position()-1,d=ep>SearchDep?SearchDep:ep-(ep&1),num=SearchNum>ep+1?ep+1:SearchNum;//ep:空位数量,d:搜索深度,num:分支数量
				if(ep<=3||ep>=FiveChessSize*FiveChessSize-3){
					if(ep+1==FiveChessSize*FiveChessSize)return std::make_pair((unsigned short)(FiveChessSize>>1),(unsigned short)(FiveChessSize>>1));//第一步落在棋盘中间
					return chessAI_static::put(cb);//除了第一步之外的前三步和最后三步直接使用静态估值函数落子
				}
				chessboard search[SearchNum];
				long long v,searchVal[SearchNum];
				std::pair<unsigned short,unsigned short> searchPair[SearchNum];
				for(int i=0;i<SearchNum;i++)searchVal[i]=-9223372036854775807;
				srand(rand()+time(0));
	...
	long long chessAI_dfs_static::MySearch(chessboard &cb,std::unordered_map<chessboard,long long,hashChess> &mp,unsigned short deep,unsigned short ep,long long large,long long small){//深度优先搜索-我方落子
		...
		unsigned short num=ep<SearchNum?ep:SearchNum;//搜索分支数量
		long long t,searchVal[SearchNum];
		chessboard search[SearchNum];
		for(int i=0;i<SearchNum;i++)searchVal[i]=-9223372036854775807;
		deep--;ep--;
		for(unsigned short x=0;x<FiveChessSize;x++)for(unsigned short y=0;y<FiveChessSize;y++)//遍历棋盘，寻找较优的搜索点
